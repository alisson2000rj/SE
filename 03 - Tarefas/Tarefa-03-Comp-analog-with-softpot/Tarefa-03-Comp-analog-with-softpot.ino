...
void setup() {
  ...
  DDRB |= 0b00100000;     //pinMode(13, OUTPUT) ou bit 5 do registrador DDRB - PB5; ligado ao LED
  DDRC |= (0 << 0);       //pinMode(A0, INPUT) ou bit 0 do registrador DDRC - PC0 ; ligado ao terminal cursor (pino variavel) do potenciometro 
  DDRC |= (0 << 1);       //pinMode(A1, INPUT) ou bit 1 do registrador DDRC - PC1 ; ligado ao 3.3v do arduino
}
void loop() {
  Serial.print("ACO : ");

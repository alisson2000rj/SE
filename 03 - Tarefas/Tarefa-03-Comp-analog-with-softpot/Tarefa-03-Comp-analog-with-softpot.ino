...
void setup() {
  Serial.begin(9600);
  ACSR &= ~(1<<7);     //Ativa o Comparador Analógico. O bit 7 (ACD) configurado com 1 = off - com 0 = ON. 
  DDRB |= 0b00110000;     //pinMode(13, OUTPUT) ou bit 5 do registrador DDRB - PB5 e pinMode(12, OUTPUT) ou bit 4 do registrador DDRB - PB4
  DDRC |= (1 << 0);       //pinMode(A0, INPUT) ou bit 0 do registrador DDRC - PC0 ; ligado ao terminal cursor (pino variavel) do potenciometro 
  DDRC |= (1 << 1);       //pinMode(A1, INPUT) ou bit 1 do registrador DDRC - PC1 ; ligado ao 3.3v do arduino
}
void loop() {
  Serial.print("ACO : ");

...
- Registradores de 8bits
 ...
 PORTxn:[w] novo valor ou configuração pull up do pino n na porta x  //  0 LOW - 1 HIGH
...
void sevenSegWrite(byte digit)  
{
  ...
  { 
    ...
    if (pin <=7) // ate o pino 7 utiliza-se o registrador PORTD
    {
      if(seven_seg_digits[digit][segCount] == 1)
      {
        PORTD |= (1<<pin); // Envia sinal HIGH pelo pino x ou bit x do registrador PORTD - Legenda: 0 LOW - 1 HIGH
      }
      else
      {
        PORTD &= ~(1<<pin); // Envia sinal LOW pelo pino x ou bit x do registrador PORTD - Legenda: 0 LOW - 1 HIGH
      }
    }
    else // acima do pino 7 utiliza-se o registrador PORTB
    {
     if(seven_seg_digits[digit][segCount] == 1)
      {
        PORTB |= (1<<0); // Envia sinal HIGH pelo pino 8 ou bit 0 do Registrador PORTB - Legenda: 0 LOW - 1 HIGH
      }
      else
      {
        PORTB &= ~(1<<0); // Envia sinal LOW pelo pino 8 ou bit 0 do Registrador PORTB - Legenda: 0 LOW - 1 HIGH
      }
    }
    ++pin;
  }
  if(digit >= 6){ PORTB |= (1<<1);  //Liga o ponto, atribui o valor 1 ao bit 1 do registrador PORTB, pino 9
  }
  else{ PORTB &= ~(1<<1);  //Desliga o ponto, nega o bit 1 do registrador PORTB, pino 9 
  }
}
...
void setup() {
  ...
  */
  ...
  DDRC = (0 << 0); // Habilita o Pino A0 para realizar leitura analógica

  PORTB = 0b00011000; //digitalWrite(pinobuttonUP, HIGH) PULL-UP do pino 12 (PB4) e digitalWrite(pinobuttonDN, HIGH) PULL-UP do pino 11 (PB3)
...
void loop() 
{
  ...
  // 0-1023 para 0-100, definido em GRAPH_LENGTH  
  ...
  Serial.print("--");
  Serial.print(softPotPosition/10);
  Serial.print("--");
  Serial.println(softPotADC/100);
  
  sevenSegWrite(softPotPosition/10); // Aciona o display passando o numero que sera apresentado

  if (softPotPosition/10 >= 6)
  {
    //################################################################################################
    ...
    //compara valores do bits 4 e 3 do registrador PINB
    ...
    if ((millis() - lastDebounceTime) > debounceDelay)
    {
      ...
      {
        buttonStateUP = (PINB & (1<<4)); // Realiza comparação de valores com base na leitura do pino 12, bit4 do registrador PINB
        ...
        if(buttonStateUP == LOW)
        {
          ...
          {
            pos += (softPotPosition/10) * 4; // atribui inicialmente 20 graus e vai somando de 20 em 20
            myservo.write(pos); // envia comando para o servo motor girar 
          }
        }
      }
      if ((PINB & (1<<3)) != buttonStateDN) // Realiza comparação de valores com base na leitura do pino 11, bit3 do registrador PINB
      {
    ...
    buttonLastStateUP = (PINB & (1<<4)); // semelhante a digitalRead(12) para o pino 12  
    buttonLastStateDN = (PINB & (1<<3)); // semelhante a digitalRead(11) para o pino 11
  }
}
